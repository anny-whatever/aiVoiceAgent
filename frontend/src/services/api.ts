...
const BACKEND_URL = process.env.REACT_APP_BACKEND || "http://localhost:3001";
...
export class ApiService {
  ...
  static async getVehicleInfo(args: any): Promise<ApiResponse> {
    const response = await fetch(`${BACKEND_URL}/api/tools/get_vehicle_info?${this.getApiQueryParams()}`, {
      method: "POST",
      headers: this.getAuthHeaders(),
      body: JSON.stringify(args),
    });
