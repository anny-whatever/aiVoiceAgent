    ...
    switch (event.type) {
      ...
      case 'output_audio_buffer.started':
        ...
        if (this.args.micRef.current) {
          this.args.micRef.current.getAudioTracks().forEach((track: MediaStreamTrack) => {
            track.enabled = false;
          });
        }
        this.args.aiDing();
  ...
  private async handleUserInfoCall(event: any) {
    try {
      ...
      if (this.args.dcRef.current) {
        ...
        // Send user data to AI for proper description
        const functionOutput = {
          message: `User information: ${result.name || 'Unknown user'}`,
          userInfo: result || {}
        };
        this.args.sendFunctionResult(
          this.args.dcRef.current,
